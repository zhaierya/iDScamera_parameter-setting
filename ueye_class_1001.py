...
class Camera(object):
    ...
    def __init__(self):
        ...
        self.img_buffers = []  # from pyueye camere
        self.num_images = 5  # the number of bilder

    def configure(self):
        # trigger or not trigger  we must do these configures before getting the images
        self.connect()
        self.get_data()
        self.sensor_info()
        # self.reset_camera()  # resettodefault  question???  : we dont  need it because we want set the parameter ourselves
        # or we should reset all parameters first
    ...
    def reset_camera(self):
        # resets all parameters to the camera-specific defaults as speicified by the driver.
        # by default, the camera uses all resolution, a medium speed and color level gain values
        # adapted to the daylight exposure
    ...
    """
    def det_area_of_interest(self):
        check(ueye.is_AOI(self.hCam, ueye.IS_AOI_IMAGE_SET_AOI,self.rectAOI, ueye.sizeof(self.rectAOI)))
    """
    ...
    def allocate_memory_for_image(self):
        ...
        if nRet != ueye.IS_SUCCESS:
            ...
            else:  # why set the color mode more one again??
                # Set the desired color mode
                nRet = ueye.is_SetColorMode(self.hCam, self.m_nColorMode)

    def allocate_memory_for_images_sequences(self, num_images=5):  # the num of images can be changed

        # for num in range(num_images):
        # mem = []
        # new_mem= self.allocate_memory_for_image()
        # mem.append(new_mem)
        # ueye.AddToSequence(self.hCam, new_mem.img_mem,
        #  len(mem))
        # self._mem.append(new_mem)
        # ueye.AddToSequence(self.hCamcam, new_mem.img_mem,
        # len(self._mem))
        # check that already existing buffers are free
        for buff in self.img_buffers:
            check(ueye.is_FreeImageMem(self.hCam, self.pcImageMemory, self.MemID))
        self.img_buffers = []
        # create asked buffers
        for i in range(num_images):
            ueye.is_AllocImageMem(self.hCam, self.rectAOI.s32Width, self.rectAOI.s32Height, self.nBitsPerPixel,
                                  self.pcImageMemory, self.MemID)
            ueye.is_AddToSequence(self.hCam, self.pcImageMemory, self.MemID)
    ...
    def set_external_trigger(self):
        ...
        if nRet != ueye.IS_SUCCESS:
            print("set_exterenal_trigger: SetExternalTrigger ERROR")
        print('External trigger mode be setted as: ', trigmode)

    def set_trigger_counter(self, nValue):
        return ueye.is_SetTriggerCounter(self.hCam, nValue)

    def get_triggermode(self):
        ret = ueye.is_SetExternalTrigger(self.hCam, ueye.IS_GET_EXTERNALTRIGGER)
        return UEyeTriggerMode(ret)  # uEye-master > driver
        # raise NotImplementedError

    def force_external_triggersignal(self):
        # a trigger is activated either via the camera's trigger pin or via a call to is_ForceTrigger()
        # is_ForceTrigger force a software-controlled capture of an image while a capturing process triggered by hardware is in progress
        # is_ForceTrigger can only be used if the triggere capturing process was started using the ***IS_DONT_WAIT** parameter

        param = ctypes.c_ulong(ueye.IS_GET_STATUS)
        ueye.is_CameraStatus(self.hCam, ueye.IS_TRIGGER_MISSED, param)
        check(ueye.is_ForceTrigger(self.hCam))
        #print('the forced external triggersignal is already sucessfully sent')

    def trigger_capture_start(self, images=1, wait=False):  # capturing and displaying frame sequences
        # is_FreezeVideo makes the camera ready for triggering just once
        # is_CaptureVideo makes the camera ready for triggering continuously
        # ??? question: how many times in calibrationGUI should be triggered ????
        # self._alloc_images(images)
        self.allocate_memory_for_images_sequences(5)  # 5: the number of to be got images
        trigger = self.get_triggermode()
        if trigger == UEyeTriggerMode.OFF:
            raise NotImplementedError
        else:
            wait_param = ueye.IS_DONT_WAIT if wait else ueye.IS_DONT_WAIT
            ueye.is_CaptureVideo(self.hCam, wait_param)
            # self._enable_event(ueye.IS_SET_EVENT_FRAME,  # A new image is available.
            #     self._callback_wrapper(callback)) # pyueye_driver
            ...
            nRet = ueye.is_InquireImageMem(self.hCam, self.pcImageMemory, self.MemID,
                                           self.rectAOI.s32Width, self.rectAOI.s32Height, self.nBitsPerPixel,
                                           self.pitch)
            if nRet != ueye.IS_SUCCESS:
                print("trigger_capture_start: is_InquireImageMem ERROR")
            else:
                print("trigger video activated. ")
                self.take_image()
                self.release_memory()
            """
    ...
    def queue_mode(self):
        ...
        pcImageMemory: pointer to the starting address of the image as allocated by is_AllocImageMem()
        MemID : ID of the image memory as allocated by is_AllocImageMem()
        pitch : the line increment of the image memory
        """
        ...
        if nRet != ueye.IS_SUCCESS:
            print("queue_mode: is_InquireImageMem ERROR")
    ...
    def triggered_video(self):
        ...
        #self.trigger_capture_start()
        ueye.is_CaptureVideo(self.hCam, ueye.int(1000)) # is_wait after timeout
        # ueye.is_FreezeVideo(self.hCam, ueye.IS_DONT_WAIT)

        self.queue_mode()
        #self.force_external_triggersignal()
...
if __name__ == "__main__":
    """
    cam = Camera()
    print('cam',cam,type(cam)) #<__main__.Camera object at 0x00000218F3C88490> <class '__main__.Camera'>
    cam.configure()
    cam.det_area_of_interest()
    cam.allocate_memory_for_image()
    #cam.allocate_memory_for_images_sequences()
    # sometimes we should adjust the other paramters, to get a appropriate exposure time, the adjusting sequence
    ...
    def open_iDScamera_in_different_mode(mode,exposure_T = 50):
        cam = Camera()
        print('cam', cam, type(cam))  # <__main__.Camera object at 0x00000218F3C88490> <class '__main__.Camera'>
        cam.configure()
        cam.det_area_of_interest()
        ...
        # cam.get_fps()
        #cam.set_exposure(50)
        cam.set_exposure(exposure_T)
        if mode =='live':
            cam.activate_live_video()
        elif mode == 'trigger':
            cam.triggered_video()
        else:
            print('mode must be live or trigger!')
        cam.close_connection

    #open_iDScamera_in_different_mode('live',50)
    open_iDScamera_in_different_mode('trigger',50)
